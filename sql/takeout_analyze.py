...
class GooglePhoto(object):
    def parse_filesystem_info(case, dic_google_photo,file_info):
        ...
        parentpath = 'Google 포토' + album_name
        ...
        if GooglePhoto.is_exist_record(dic_google_photo['filepath'],case.preprocess_db_path)==0:
            ...
            query = 'UPDATE parse_google_photo set \
                                ...
                                WHERE filepath = "%s"' % \
                    (dic_google_photo['parentpath'], dic_google_photo['album_name'], dic_google_photo['filename'],
                     dic_google_photo['extension'], \
                     int(dic_google_photo['bytes']), int(dic_google_photo['file_modified_time']),
                     dic_google_photo['filepath'])
            SQLite3.execute_commit_query(query, case.preprocess_db_path)

    # ---------------------------------------------------------------------------------------------------------------
    def parse_google_photo_json(case, dic_google_photo, file_info):
        ...
        if GooglePhoto.is_exist_record(dic_google_photo['filepath'], case.preprocess_db_path) == 0:
            ...
            query = 'UPDATE parse_google_photo set \
                    ...
                    WHERE filepath = "%s"' % \
                    (int(dic_google_photo['photo_created_time']), int(dic_google_photo['photo_modified_time']),
                     int(dic_google_photo['photo_taken_time']), \
                     dic_google_photo['latitude'], dic_google_photo['longitude'], dic_google_photo['latitude_span'],
                     dic_google_photo['longitude_span'], \
                     dic_google_photo['exif_latitude'], dic_google_photo['exif_longitude'],
                     dic_google_photo['exif_latitude_span'], dic_google_photo['exif_longitude_span'],
                     dic_google_photo['filepath'])
            SQLite3.execute_commit_query(query, case.preprocess_db_path)

    # ---------------------------------------------------------------------------------------------------------------
    def parse_google_photo_album_json(case, file_info, list_album_info):
        data = json.load(open(file_info, 'r', encoding='utf-8'))
        filename = os.path.basename(file_info)
        album_name = file_info.split(case.takeout_google_photo_path, 1)[1].strip(filename)
        album_name = album_name.split(os.sep)[1]
    ...
    def insert_log_info_to_preprocess_db(dic_google_photo, preprocess_db_path):
        query = 'INSERT INTO parse_google_photo \
                ...
                (dic_google_photo['parentpath'], dic_google_photo['album_name'], dic_google_photo['filename'],
                 dic_google_photo['extension'], int(dic_google_photo['bytes']), \
                 int(dic_google_photo['album_created_time']), int(dic_google_photo['photo_taken_time']),
                 int(dic_google_photo['photo_created_time']), int(dic_google_photo['photo_modified_time']),
                 int(dic_google_photo['file_modified_time']), \
                 dic_google_photo['latitude'], dic_google_photo['longitude'], dic_google_photo['latitude_span'],
                 dic_google_photo['longitude_span'], \
                 dic_google_photo['exif_latitude'], dic_google_photo['exif_longitude'],
                 dic_google_photo['exif_latitude_span'], dic_google_photo['exif_longitude_span'],
    ...
    def parse_google_photo(case):
        ...
        for i in trange(len(list_filepath), desc="[Parsing the Google Photo Data......................]", unit="epoch"):
            ...
            dic_google_photo = {'parentpath': "", 'album_name': "", 'filename': "", 'extension': "", 'bytes': 0, \
                                'album_created_time': 0, 'photo_taken_time': 0, 'photo_created_time': 0,
                                'photo_modified_time': 0, 'file_modified_time': 0, \
                                'latitude': "", 'longitude': "", 'latitude_span': "", 'longitude_span': "", \
                                'exif_latitude': "", 'exif_longitude': "", 'exif_latitude_span': "",
                                'exif_longitude_span': "", 'filepath': ""}
            ...
            if filename == 'metadata.json':
                GooglePhoto.parse_google_photo_album_json(case, list_filepath[i], list_album_info)
            elif extension == 'json':
                GooglePhoto.parse_google_photo_json(case, dic_google_photo, list_filepath[i])
            else:
                GooglePhoto.parse_filesystem_info(case, dic_google_photo, list_filepath[i])

        for i in trange(len(list_album_info), desc="[Parsing the Google Photo Data(Album information)...]",
                        unit="epoch"):
